...
@if(session('success'))
<div class="mb-6 p-4 rounded-xl border flex items-center justify-between shadow-sm"
  style="background-color: #e6f4ea; border-color: #34a853;">
  <div class="flex items-center gap-3">
    <span class="material-symbols-outlined font-bold" style="color: #137333;">check_circle</span>
    <div>
...
<div class="flex-grow p-4 sm:p-6 lg:p-8 overflow-y-auto scrollbar-lux space-y-8 sm:space-y-12">
  ...
  <div class="form-card bg-white border border-secondary/20 rounded-2xl shadow-lg overflow-hidden">
    ...
    <div class="p-5 sm:p-8 md:p-10">
      <form id="checkoutOrderForm" class="space-y-8" action="{{ route('receptionist.insert.order') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label for="client_id" class="block text-[10px] font-bold uppercase tracking-widest mb-2" style="color:#0b3c2c;">
          ...
          <div>
            <label for="staff_id" class="block text-[10px] font-bold uppercase tracking-widest mb-2" style="color:#0b3c2c;">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 field-cream border rounded-lg p-6 max-h-[280px] overflow-y-auto">
            @for ($i = 1; $i <= 3; $i++)
              <div>
                ...
                <div class="relative">
                  <select name="service_id_{{ $i }}" id="service_id_{{ $i }}"
                    class="w-full bg-background border border-secondary/30 rounded-lg px-4 py-3 text-sm focus:border-primary focus:ring-1 focus:ring-primary appearance-none cursor-pointer"
                    {{ $i === 1 ? 'required' : '' }}>
                    <option value="">Select Service {{ $i }}</option>
                    @foreach ($services as $service)
                      <option value="{{ $service->id }}" data-price="{{ $service->total_price ?? $service->price ?? 0 }}">
                        {{ $service->name }} — ${{ number_format($service->total_price ?? $service->price ?? 0, 2) }}
                      </option>
                    @endforeach
                  </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50/50 p-6 border rounded-xl">
          <div>
            <span class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Subtotal</span>
            <span class="text-lg font-bold text-gray-700 font-serif" id="subtotal-display">$0.00</span>
          </div>
          <div>
            <span class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Estimated Tax (8%)</span>
          ...
          <div class="md:border-l md:pl-6 border-gray-200">
            <span class="block text-[10px] font-bold uppercase tracking-widest mb-1" style="color:#0b3c2c;">Total Amount Due</span>
  ...
  <div class="mt-8 sm:mt-12">
    <h2 class="font-serif text-2xl sm:text-3xl font-bold mb-4 sm:mb-6" style="color:#0b3c2c;">Recent Orders</h2>
      ...
      <div class="overflow-x-auto scrollbar-lux">
        <table class="w-full text-left border-collapse min-w-[750px]">
          ...
          <tbody class="divide-y divide-gray-100 text-sm">
            @forelse(\App\Models\Orders::with(['client','stylist','service1','service2','service3'])->latest()->take(20)->get() as $order)
            <tr class="hover:bg-gray-50/50 transition-colors">
              <td class="py-4 px-6 font-semibold text-gray-700">#{{ $order->id }}</td>
              <td class="py-4 px-6 text-gray-800 font-medium">{{ $order->client->name ?? $order->client->full_name ?? '—' }}</td>
              <td class="py-4 px-6 text-gray-600">{{ $order->stylist->full_name ?? '—' }}</td>
              <td class="py-4 px-6 text-gray-600 max-w-xs truncate">
                {{ collect([$order->service1->name ?? null, $order->service2->name ?? null, $order->service3->name ?? null])->filter()->implode(', ') ?: '—' }}
              </td>
              <td class="py-4 px-6 font-bold font-serif" style="color:#0b3c2c;">${{ number_format($order->total_amount, 2) }}</td>
              <td class="py-4 px-6 text-gray-500 text-xs">{{ $order->created_at->format('M d, Y') }}</td>
            </tr>
            @empty
            <tr>
              <td colspan="6" class="py-8 px-6 text-center text-gray-400 text-sm">No orders recorded yet.</td>
            </tr>
            @endforelse
          </tbody>
        </table>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    serviceSelects.forEach(s => { if (s) s.addEventListener('change', calculateTotals); });
    if (checkoutForm) checkoutForm.addEventListener('reset', () => setTimeout(calculateTotals, 50));
  });
</script>
@endsection