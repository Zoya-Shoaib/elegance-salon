        ...
        <tbody class="divide-y divide-gray-100 text-sm">
          @forelse($orders as $order)
            <tr class="hover:bg-gray-50/50 transition-colors">
              ...
              <td class="py-4 px-6 text-gray-800 font-medium">{{ $order->client->name ?? $order->client->full_name ?? 'N/A' }}</td>
              <td class="py-4 px-6 text-gray-600">{{ $order->stylist->full_name ?? $order->stylist->name ?? 'N/A' }}</td>
              <td class="py-4 px-6 text-gray-600 max-w-xs truncate">
                 {{ $order->service1->name ?? '' }} 
                 {{ isset($order->service2) ? ', ' . $order->service2->name : '' }} 
                 {{ isset($order->service3) ? ', ' . $order->service3->name : '' }}
                 @if(!$order->service_id_1 && !$order->service_id_2 && !$order->service_id_3) General Service @endif
              </td>
          ...
          @empty
            <tr>
              <td colspan="6" class="py-4 px-6 text-center text-gray-500">No orders found.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
