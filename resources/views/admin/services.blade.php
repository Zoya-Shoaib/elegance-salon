...
@endfor
  ...
  <!-- Services List Table -->
  <div class="mt-8 bg-white border border-secondary/20 rounded-2xl shadow-lg overflow-hidden">
    <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-background/50">
      <h4 class="font-serif text-lg font-bold text-primary">Existing Services</h4>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-[#FAF8F3]/50 text-[10px] uppercase tracking-wider text-gray-500 border-b border-gray-100">
            <th class="p-4 font-bold">Service Name</th>
            <th class="p-4 font-bold">Category</th>
            <th class="p-4 font-bold">Base Price</th>
            <th class="p-4 font-bold">Products Used</th>
          </tr>
        </thead>
        <tbody class="text-sm divide-y divide-gray-50 font-sans">
          @forelse($services as $service)
            <tr class="hover:bg-gray-50/50">
              <td class="p-4 font-semibold text-primary">{{ $service->name }}</td>
              <td class="p-4 text-gray-600">{{ ucfirst($service->category) }}</td>
              <td class="p-4 text-gray-600">${{ number_format($service->base_price, 2) }}</td>
              <td class="p-4 text-gray-600">
                @if($service->product_id_1) P1: {{ $service->product1->name ?? $service->product_id_1 }}<br> @endif
                @if($service->product_id_2) P2: {{ $service->product2->name ?? $service->product_id_2 }}<br> @endif
                @if($service->product_id_3) P3: {{ $service->product3->name ?? $service->product_id_3 }} @endif
                @if(!$service->product_id_1 && !$service->product_id_2 && !$service->product_id_3) None @endif
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="4" class="p-4 text-center text-gray-400 text-xs">No services added yet.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
